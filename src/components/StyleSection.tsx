...
const StyleOption: React.FC<StyleOptionProps> = ({ style, isSelected, onClick, index }) => (
    <ScaleIn delay={index * 50}>
        <button
            onClick={() => onClick(style.value)}
            className={`group relative p-6 rounded-2xl text-center transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-white/20 ${isSelected
        ...
        >
            <div className={`w-12 h-12 mx-auto mb-3 rounded-xl bg-gradient-to-r ${style.gradient} ${isSelected ? 'opacity-30' : 'opacity-100'
                } transition-opacity duration-300`} />

            <h3 className="font-semibold text-lg mb-1">{style.label}</h3>
            <p className={`text-sm transition-opacity duration-300 ${isSelected ? 'opacity-90' : 'opacity-70'
                }`}>
                {style.description}
            </p>
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex bg-white/10 rounded-lg p-1">
                        ...
                        <button
                            ...
                            className={`px-3 py-1 rounded text-sm transition-all duration-200 ${viewMode === 'dropdown'
                                ? 'bg-white/20 text-white'
                                : 'text-white/60 hover:text-white/80'
                                }`}
                        >
