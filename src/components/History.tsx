...
// Grid Item Component for Landing Page
const GridHistoryItem = ({ item, isSelected, onSelect, index }) => {
  ...
  return (
    <FadeInUp delay={index * 100}>
      <div 
        ...
        className={`group backdrop-blur-xl bg-black/20 border border-white/20 rounded-2xl p-6 cursor-pointer transition-all duration-300 hover:bg-black/30 hover:scale-105 hover:border-white/30 ${
          isSelected ? 'ring-2 ring-orange-500 bg-black/40' : ''
        }`}
      >
...
const ListHistoryItem = ({ item, isSelected, onSelect, index }) => {
  const formatTime = (timestamp) => {
    return new Date(timestamp).toLocaleTimeString();
  };
  ...
  return (
    <FadeInUp delay={index * 50}>
      <button
        onClick={() => onSelect(item)}
        className={`w-full p-4 rounded-xl border-2 transition-all duration-300 text-left focus:outline-none focus:ring-4 focus:ring-orange-500/30 hover:scale-[1.02] hover:shadow-lg group ${
          isSelected
...
const HistoryControls = ({ searchTerm, onSearchChange, selectedFilter, onFilterChange, viewMode, onViewModeChange, showControls = true }) => {
