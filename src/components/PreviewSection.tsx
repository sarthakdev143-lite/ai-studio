...
const ImagePreview: React.FC<ImagePreviewProps> = ({ src, alt, title, isEmpty = false, children }) => (
...
const ActionButton: React.FC<ActionButtonProps> = ({ icon: Icon, label, onClick, variant = 'secondary' }) => (
    <button
        onClick={onClick}
        className={`flex items-center gap-2 px-4 py-2 rounded-xl font-medium transition-all duration-200 hover:scale-105 ${variant === 'primary'
                                    ...
                                    <SlideInFromLeft delay={200}>
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3">
                                                <div className={`w-4 h-4 rounded bg-gradient-to-r ${STYLE_OPTIONS.find(s => s.value === (selectedHistoryItem?.style || selectedStyle))?.gradient || 'from-gray-500 to-gray-600'
                                                    }`} />
